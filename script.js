...
function saveClothes(){
  try{
    ...
    toast("Stockage plein 😬 Supprime une ancienne pièce ou réessaie avec une photo plus légère.");
    return false;
  }
}
...
/* Redimensionne + recompresse une photo (canvas → JPEG) avant stockage.
   Indispensable pour les photos iPhone (souvent 3-8 Mo, parfois en HEIC) :
   sans ça, le localStorage (~5 Mo sur Safari iOS) sature dès la 1ère photo
   et l'ajout échoue silencieusement. */
...
photoInput.addEventListener("change", ()=>{
  ...
  photoDropLabel.textContent = "⏳ Chargement de la photo…";
  const reader = new FileReader();
  reader.onload = async (e)=>{
    try{
      const compact = await compressImage(e.target.result, 1000, 0.72);
      photoPreview.src = compact;
      photoPreview.hidden = false;
      photoDropLabel.hidden = true;
    }catch(err){
      photoDropLabel.hidden = false;
      photoDropLabel.textContent = "📸 Ajouter une photo";
      toast("Cette photo n'a pas pu être lue, réessaie avec une autre.");
    }
  };
  reader.onerror = ()=>{
    photoDropLabel.hidden = false;
...
document.getElementById("btnAddClothing").addEventListener("click", ()=>{
  ...
  if(state.editingId){
    ...
    if(!saveClothes()){ item.image = previousImage; return; } // échec stockage → on n'efface pas le formulaire
    ...
    if(!saveClothes()){ state.clothes.pop(); return; } // échec stockage → on n'efface pas le formulaire
